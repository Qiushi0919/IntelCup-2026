...
void UpdateStatus()
{
    ...
    switch (FollowManager.ActionList)
    {
        ...
        case ActionGoForward:
            ActionFormChange(5, LeftTtype, ActionHoldLeftTtype);
            break;
        case ActionHoldLeftTtype:
            ActionHoldPoint(MAX_HOVER_ERR, 800, ActionLand);
            break;
...
void UpdateAction(float dt)
{
    ...
    case ActionGoForward:
        ...
        break;
    case ActionHoldLeftTtype:
        if (FollowManager.ptrFrame->FormType == LeftTtype)
        {
            PIDGroup[emPID_FolloLinePosVertically].desired = 120 / 2;
            PIDGroup[emPID_FolloLinePosHorizontally].desired = 160 / 2;

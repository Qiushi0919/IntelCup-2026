...
from telemetry_service import TelemetryThread
...
class GroundStationWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # 初始化3D仿真视图
        self.drone_3d.set_waypoints(self.simulator.waypoints)
        self.drone_3d.start()
    ...
    def _build_docks(self) -> None:
        ...
        # ---- 飞控指令面板 ----
        self.flight_ctrl = FlightControlPanel()
        self.flight_ctrl.command_sent.connect(self._on_flight_command)
        self._add_dock(
            "flightctrl",
            "飞控指令",
            self.flight_ctrl,
    ...
    def _on_state_changed(self, state: DroneState) -> None:
        ...
        # 更新3D仿真视图
        self.drone_3d.set_drone_state(
            x=state.x, y=state.y, z=state.altitude,
            roll=state.roll, pitch=state.pitch, yaw=state.yaw
        )

    def _on_flight_command(self, cmd: str) -> None:
        """处理飞控指令"""
        ok, msg = self.simulator.apply_command(cmd)
        if ok:
            self._append_log("INFO", f"指令成功: {msg}")
            if hasattr(self, 'flight_ctrl'):
                self.flight_ctrl.log.setText(f"✅ {msg}")
        else:
            self._append_log("WARNING", f"指令失败: {msg}")
            if hasattr(self, 'flight_ctrl'):
                self.flight_ctrl.log.setText(f"❌ {msg}")
    ...
    def connect_telemetry(self, port: str, baudrate: int, protocol: str) -> None:
        port = port.strip()
        ...
        self.device_panel.set_telemetry_status(False, f"正在连接 {port} @ {baudrate}")
        self._append_log("INFO", f"正在连接飞控数传：{port} @ {baudrate}，NCLink 只读")
    ...
    def _refresh_self_check_view(self) -> None:
        ...
        if "陀螺" in label or "惯导" in label or "姿态" in label:
            focus_line = (

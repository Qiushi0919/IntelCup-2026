...
void remotectrl::timer_event()
{
    ...
    ui->ch8->setText(QString::number(channel[8]));
    ui->ch9->setText(QString::number(channel[9]));
    ...
    channel_package[8]=channel[8];
    channel_package[9]=channel[9];
    ...
    Send_RC(channel_package[0],channel_package[1],channel_package[2],channel_package[3],
            ...
            channel_package[8],channel_package[9],
...
void remotectrl::Send_RC(uint16_t ch0,uint16_t ch1,uint16_t ch2,uint16_t ch3,uint16_t ch4,uint16_t ch5,uint16_t ch6,uint16_t ch7,uint16_t ch8,uint16_t ch9,uint8_t unlock_flag,uint8_t takeoff_flag)//地面站指令发送RC数据
{

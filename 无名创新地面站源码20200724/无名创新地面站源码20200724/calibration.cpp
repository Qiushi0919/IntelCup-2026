...
void calibration::timer_show()
{
  ...
  for(uint16_t i=0;i<8;i++)
  {
     if(rc[i]>rc_max[i])  rc_max[i]=rc[i];
     if(rc[i]<rc_min[i])  rc_min[i]=rc[i];
  }

  ui->ch0_max->setText(QString::number(rc_max[0]));
  ui->ch1_max->setText(QString::number(rc_max[1]));
  ui->ch2_max->setText(QString::number(rc_max[2]));
  ui->ch3_max->setText(QString::number(rc_max[3]));
  ui->ch4_max->setText(QString::number(rc_max[4]));
  ui->ch5_max->setText(QString::number(rc_max[5]));
  ui->ch6_max->setText(QString::number(rc_max[6]));
  ui->ch7_max->setText(QString::number(rc_max[7]));

  ui->ch0_min->setText(QString::number(rc_min[0]));
  ui->ch1_min->setText(QString::number(rc_min[1]));
  ui->ch2_min->setText(QString::number(rc_min[2]));
  ui->ch3_min->setText(QString::number(rc_min[3]));
  ui->ch4_min->setText(QString::number(rc_min[4]));
  ui->ch5_min->setText(QString::number(rc_min[5]));
  ui->ch6_min->setText(QString::number(rc_min[6]));
  ui->ch7_min->setText(QString::number(rc_min[7]));
}
...
{
  if(Send_CAL(0x03,0x00,0x00)==true)
  {
      QDateTime current_date_time = QDateTime::currentDateTime();
      QString current_time = current_date_time.toString("hh:mm:ss.zzz");
...
void calibration::on_rc_cal_start_clicked()
{
  for(uint16_t i=0;i<8;i++)
  {
     rc_min[i]=1500;
  ...
  if(Send_CAL(0x05,0x00,0x00)==true)
  {
      QDateTime current_date_time = QDateTime::currentDateTime();
      QString current_time = current_date_time.toString("hh:mm:ss.zzz");
      ui->msg->appendPlainText(current_time+"毫秒>遥控器校准指令已发送");
      ui->msg->moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
  }
}

void calibration::on_rc_cal_save_clicked()
{
    if(Send_CAL(0x06,0x00,0x00)==true)
    {
        QDateTime current_date_time = QDateTime::currentDateTime();
        QString current_time = current_date_time.toString("hh:mm:ss.zzz");
        ui->msg->appendPlainText(current_time+"毫秒>遥控器保存数据指令已发送");
        ui->msg->moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
    }
}

void calibration::on_bt1_mag_cal_clicked()
{
    if(Send_CAL(0x00,0x01,0x00)==true)
    {
        QDateTime current_date_time = QDateTime::currentDateTime();
        QString current_time = current_date_time.toString("hh:mm:ss.zzz");
        ui->msg->appendPlainText(current_time+"毫秒>磁力计校准-水平旋转指令已发送");
        ui->msg->moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
    }
}

void calibration::on_bt2_mag_cal_clicked()
{
    if(Send_CAL(0x00,0x02,0x00)==true)
    {
        QDateTime current_date_time = QDateTime::currentDateTime();
        QString current_time = current_date_time.toString("hh:mm:ss.zzz");
        ui->msg->appendPlainText(current_time+"毫秒>磁力计校准-机头朝上旋转指令已发送");
        ui->msg->moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
    }
}

void calibration::on_bt3_mag_cal_clicked()
{
    if(Send_CAL(0x00,0x03,0x00)==true)
    {
        QDateTime current_date_time = QDateTime::currentDateTime();
        QString current_time = current_date_time.toString("hh:mm:ss.zzz");
        ui->msg->appendPlainText(current_time+"毫秒>磁力计校准-右侧朝上旋转指令已发送");
        ui->msg->moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
    }
}
